...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (!empty($_POST['url_video']) && !empty($_POST['judul_video']) && !empty($_POST['kategori_video']) && !empty($_POST['durasi_video']) && !empty($_POST['sinopsis_video']) && !empty($_POST['deskripsi_video']) && !empty($_POST['img_thumbnail'])) {
      ...
      // Lakukan query untuk menambahkan data baru ke dalam database
      // $sql = "INSERT INTO vidio (url_vidio, judul_vidio, id_vidio, durasi_vidio, sinopsis_vidio, deskripsi_vidio, img_thumbnail)
      // VALUES ('$url_video', '$judul_video', (SELECT id_kategori FROM kategori WHERE nama_kategori = '$kategori_video'), '$durasi_video', '$sinopsis_video', '$deskripsi_video', '$img_thumbnail')";
      $sql="INSERT INTO vidio INNER JOIN kategori ON vidio.id_vidio = kategori.id_kategori
      SET vidio.url_vidio='$url_video', 
          ...
          // error masih belum betul querynya
...
<div class="container-fluid pb-4">
  <div class="card shadow">
    ...
    <form class="px-5 pt-4 pb-5" method="POST" action="" enctype="multipart/form-data">
      ...
      <div class="row">
      <label class="form-label" for="customFile">Uploud Gambar Gan</label>
